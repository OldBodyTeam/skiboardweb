...
const DrawList = () => {
  ...
  const handleDeleteEffects = (e: MouseEvent<HTMLElement>, collectionId: string, frameIndex: number) => {
    e.stopPropagation();
    if (window.ReactNativeWebView) {
      window.ReactNativeWebView.postMessage(
        JSON.stringify({ type: 'delete', deleteInfo: { collectionId, frameIndex } }),
      );
    } else {
      const postMessage = window.parent.postMessage;
      postMessage(JSON.stringify({ type: 'delete', deleteInfo: { collectionId, frameIndex } }));
    }
  };
              ...
              <div className="w-full h-full bg-[rgba(118,118,118,0.1)] rounded-[40px] relative">
                ...
                {currenStatus ? (
                  <div
                    ...
                    onClick={(e) => handleDeleteEffects(e, itemData.id, index)}
                  >
